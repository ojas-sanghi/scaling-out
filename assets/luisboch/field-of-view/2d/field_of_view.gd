...
export var radius_warn = 350
export var radius_danger = 350
...
func check_view():
	...
	for i in range(view_detail + 1):
		...
		var result = space_state.intersect_ray(
			get_global_transform().origin, to_global(point), [get_parent()]
		)
...
#					cs_events_node.Instance.publishLevelFailed()
					...
					var view_angle = rad2deg(
						deg_to_vector(rad2deg(get_global_transform().get_rotation()) + 90).angle_to(
							tgt_dir
						)
					)
